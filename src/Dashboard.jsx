  ...
  return (
    <div className="d-flex flex-column align-items-center">
      ...
      <svg width="84" height="84" viewBox="0 0 84 84">
        ...
        <circle 
          cx="42" cy="42" r={radio} stroke={colorProgreso} strokeWidth="6" fill="none"
    ...
    <div className="curso-card h-100 p-4">
      ...
      <div className="row g-0">
        ...
        <div className="col-8 ps-3">
          <div className="d-flex justify-content-between align-items-center mb-2 px-2 py-1 rounded" style={{ backgroundColor: "#f8fafc" }}>
            <span className="fw-bold text-xs" style={{ color: "#334155" }}>Calificaciones</span>
              ...
              return (
                <li key={index}>
                  <span className="d-flex align-items-center gap-2 text-muted" style={{ color: "#475569" }}>
                    {estaCalificado ? <CheckCircle2 size={12} color="#22c55e" /> : <Circle size={12} color="#cbd5e1" />}
                    {evaluacion.label}
                  </span>
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // ✅ PASO 1: Todos los cursos en paralelo (antes era uno por uno)
        const cursosSnaps = await Promise.all(
          cursosVinculados.map((id) => getDoc(doc(db, "cursos", id)))
        );

        const cursosData = cursosSnaps
          .filter((s) => s.exists())
          .map((s) => ({ id: s.id, ...s.data() }));

        // ✅ PASO 2: Docentes únicos + notas en paralelo (dos requests simultáneos)
        const docenteIdsUnicos = [
          ...new Set(cursosData.map((c) => c.docente_id).filter(Boolean))
        ];

        const [docentesSnaps, notasSnapshot] = await Promise.all([
          Promise.all(docenteIdsUnicos.map((id) => getDoc(doc(db, "docentes", id)))),
          getDocs(query(collection(db, "notas"), where("alumnoId", "==", usuario.id)))
        ]);

        const docentesTemp = {};
        docentesSnaps.forEach((s) => {
          if (s.exists()) docentesTemp[s.id] = s.data().nombre;
        });
    ...
    return (
      <div className="d-flex justify-content-center align-items-center bg-light"
        ...
        <div className="text-center">
          ...
          <div className="text-muted small fw-semibold">Cargando tu información...</div>
        </div>
      </div>
    );
  }
        ...
        <div className="main-content">
          <header className="top-header">
            <div className="d-flex align-items-center">
              ...
              <h1 className="header-title d-none d-md-block">PORTAL DEL OFICIAL ALUMNO</h1>
            </div>
            <div className="user-profile shadow-sm">
              ...
              <div className="avatar-circle">{getInitials(usuario.nombre)}</div>
              <button className="btn btn-sm btn-link text-danger ms-1 p-1" onClick={onLogout} title="Cerrar Sesión" style={{ textDecoration: 'none' }}>
                <LogOut size={18} />
              </button>
            </div>
          </header>
          ...
          <main className="content-area">
            ...
            <div className="d-flex align-items-center mb-4 p-3 rounded shadow-sm bg-white" style={{ borderLeft: "4px solid #3b82f6" }}>
              ...
              <h5 className="mb-0 text-dark fw-bold" style={{ letterSpacing: '-0.3px' }}>MIS ASIGNATURAS</h5>
            </div>
              ...
              <div className="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4">
                {cursos.map((curso) => (
                  <div key={curso.id} className="col">
                    <CursoCard
                      curso={curso}
                      notas={notasMap[curso.id] || {}}
                      docenteNombre={docentesMap[curso.docente_id] || "No asignado"}
                    />
                  </div>
                ))}
              </div>
